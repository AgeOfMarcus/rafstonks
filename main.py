...
promo_keys = {x.split(":")[0]: int(x.split(":")[1])*100 for x in os.getenv("keylist").split(", ")}#{key: bal * 100 for key, bal in requests.get(os.getenv("keys")).json().items()}
...
class Stonks(object):
    ...
    def roller(self):
        ...
        while True:
            ...
            if self.current > tg and self.current > 1.02:
                break
...
# ---------------------- rewards 
'''
@socketio.on('rewards init')
def s_rewards_init(json):
    if not (user := socket_auth(json)):
        return {'status': False, 'msg': 'Bad authentication'}
    adfly = requests.get('https://adfly.marcusweinberger.repl.co/api/create').json()
    return {
        'status': True,
        'id': adfly['id'],
        'url': f'https://adfly.marcusweinberger.repl.co/flow/init/{adfly["id"]}',
    }

@socketio.on('rewards check')
def s_rewards_check(json):
    if not (user := socket_auth(json)):
        return {'status': False, 'msg': 'Bad authentication'}
    adfly = requests.post('https://adfly.marcusweinberger.repl.co/api/check', data={
        'id': json['id'],
    }).json()
    if adfly['status']:
        win = random.randint(20, 100) * int(os.getenv('RW_M'))
        user['balance'] += win * 100
        return {
            'status': True,
            'amount': win,
        }
